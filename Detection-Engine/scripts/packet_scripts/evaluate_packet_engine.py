...
# עולים שתי רמות לתיקיית השורש של הפרויקט
...
TEST_DATA_PATH = os.path.join(PROJECT_ROOT, 'data/processed/network/packet_test.npy') 
TEST_LABELS_PATH = os.path.join(PROJECT_ROOT, 'data/processed/network/packet_labels_test.npy')

# Packet Model Parameters
INPUT_DIM = 12
HIDDEN_DIM = 256

def get_latest_model_path():
    pattern = os.path.join(ARTIFACTS_DIR, "packet_model_master_*.pth")
    files = glob.glob(pattern)
    if not files: 
        raise FileNotFoundError("[-] No packet model found in artifacts directory.")
    files.sort()
    return files[-1]

def load_trained_model(device):
    model_path = get_latest_model_path()
    print(f"[INFO] Loading latest Packet Model: {os.path.basename(model_path)}")
...
def run_evaluation():
    ...
    print("[INFO] Loading Packet test data...")
    ...
    print("\n" + "🚀"*15)
    print("   PACKET MODEL: OPTIMIZED RESULTS (ENERGY)")
    print("🚀"*15)
    ...
    plot_results(y_true, anomaly_scores, opt_threshold, tp, tn, fp, fn, "Packet")
