import pandas as pd
import numpy as np
import os
import joblib
from sklearn.preprocessing import MinMaxScaler, LabelEncoder

# הגדרות נתיבים
BASE_DIR = '../Datasets/Model_3/r1/r1/' 
OUTPUT_DIR = '../data/processed/policy'
ARTIFACTS_DIR = '../models/artifacts'

# רשימת הקבצים לאיחוד (עכשיו לכולם יש Header זהה)
FILES_TO_MERGE = ['logon.csv', 'device.csv', 'http.csv']

def preprocess_unified_cert():
    all_events = []
    
    print("--- Starting Unified CERT Preprocessing ---")

    for filename in FILES_TO_MERGE:
        file_path = os.path.join(BASE_DIR, filename)
        if not os.path.exists(file_path):
            print(f"Warning: {filename} not found, skipping...")
            continue
            
        print(f"Loading {filename}...")
        # טעינה רגילה - כעת ה-Header קיים בכל הקבצים
        df = pd.read_csv(file_path)
        
        # הוספת סוג האירוע (השם של הקובץ ללא הסיומת) כמאפיין נוסף
        df['event_source'] = filename.split('.')[0]
        
        # שמירת העמודות הרלוונטיות בלבד
        all_events.append(df[['date', 'user', 'pc', 'activity', 'event_source']])

    # 1. איחוד כל האירועים לציר זמן אחד
    combined_df = pd.concat(all_events, ignore_index=True)
    print(f"Total events combined: {len(combined_df)}")

    # 2. טיפול בזמן (הפיכה למספרים שה-RBM מבין)
    combined_df['date'] = pd.to_datetime(combined_df['date'])
    combined_df['hour'] = combined_df['date'].dt.hour
    combined_df['day_of_week'] = combined_df['date'].dt.dayofweek
    
    # 3. קידוד קטגוריאלי (הפיכת טקסט למזהים מספריים)
    le_user = LabelEncoder()
    combined_df['user_id'] = le_user.fit_transform(combined_df['user'])
    
    le_pc = LabelEncoder()
    combined_df['pc_id'] = le_pc.fit_transform(combined_df['pc'])
    
    # ה-LabelEncoder יטפל גם ב-Logon/Logoff וגם בכתובות URL
    le_act = LabelEncoder()
    combined_df['activity_id'] = le_act.fit_transform(combined_df['activity'].astype(str))
    
    le_src = LabelEncoder()
    combined_df['source_id'] = le_src.fit_transform(combined_df['event_source'])

    # 4. בחירת וקטור המאפיינים הסופי
    features = combined_df[['hour', 'day_of_week', 'user_id', 'pc_id', 'activity_id', 'source_id']]
    
    # 5. נרמול לטווח [0, 1]
    scaler = MinMaxScaler()
    scaled_features = scaler.fit_transform(features)
    
    # 6. שמירה לקבצי אימון ומטה-דאטה
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(ARTIFACTS_DIR, exist_ok=True)
    
    np.save(os.path.join(OUTPUT_DIR, 'policy_train.npy'), scaled_features)
    
    joblib.dump(scaler, os.path.join(ARTIFACTS_DIR, 'policy_scaler.pkl'))
    joblib.dump(le_user, os.path.join(ARTIFACTS_DIR, 'policy_user_encoder.pkl'))
    
    print("\n--- Preprocessing Success ---")
    print(f"Matrix shape for RBM training: {scaled_features.shape}")

if __name__ == "__main__":
    preprocess_unified_cert()